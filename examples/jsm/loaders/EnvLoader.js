...
export class EnvLoader extends Loader {
		...
		loader.load(
			...
			function(buffer) {
				if (onLoad !== undefined) {
					...
					onLoad(scope.parse(srcBuffer, options));
				}
			},
			onProgress,
			onError
	...
	parse(buffer, options) {
		const info = this.GetEnvInfo(buffer);
		const imageData = this.CreateImageDataArrayBufferViews(buffer, info);
		const envMap = new TextureCube();

		let outputTexture = options.outputTexture;
		if (outputTexture) {
		...
		Promise.all(promises).then(images => {
			for (let i = 0; i < mipmapsCount; i++) {
				...
				reflectionProbe.render(options.renderer, dummyScene);
	...
	GetEnvInfo(data) {
		...
		if (manifest.specular) {
			...
			manifest.specular.lodGenerationScale =
                manifest.specular.lodGenerationScale || 0.8;
		}
