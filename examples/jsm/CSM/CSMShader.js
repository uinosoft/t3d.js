...
const CSMShader = {
    ...
    uniform float u_csmdebug;
    uniform mat4 u_ProjectionView;
    uniform mat4 u_projectionMatrix;
    #endif
    
        ` + ShaderChunk.light_pars_frag,
	light_frag: `// Use v_modelPos from modelPos_pars_frag
        // Use geometryNormal, N from normal_frag
            ...
            #endif
            #if NUM_DIR_LIGHTS > 0
                float lightDistance = length(u_CameraPosition - v_modelPos);
            #if defined( USE_CSM ) && defined( CSM_CASCADES )
                linearDepth = ((u_ProjectionView * vec4(v_modelPos, 1.0)) * u_projectionMatrix).z  / (shadowFar - cameraNear);
                falloff = 1.0;
                    ...
                    for (int i = 0; i < NUM_DIR_LIGHTS; i++) {
                        L = normalize(-u_Directional[i].direction);
                                ...
                                if( linearDepth >= csmx && linearDepth < csmy)
                                {
                                    float dist = min( linearDepth - csmx, csmy - linearDepth );
                                    float ratio = clamp( dist / margin, 0.0, 1.0 );
                                    falloff1 = getShadow(directionalShadowMap[i], vDirectionalShadowCoord[i], u_DirectionalShadow[i].shadowMapSize, u_DirectionalShadow[i].shadowBias, u_DirectionalShadow[i].shadowParams);
                                    bool shouldFadeLastCascade = UNROLLED_LOOP_INDEX == CSM_CASCADES - 1 && linearDepth > cascadeCenter;
                                    falloff *= mix(falloff, falloff1, shouldFadeLastCascade ? ratio : 1.0 );
                                }
                                #else
                                    if(linearDepth >= CSM_cascades[UNROLLED_LOOP_INDEX].x && linearDepth < CSM_cascades[UNROLLED_LOOP_INDEX].y){
                                        ...
                                        reflectedLight.indirectDiffuse.r += CSM_cascades[UNROLLED_LOOP_INDEX].y * u_csmdebug/4.;
                                    }
                                #endif 
                    ...
                    irradiance = u_Directional[CSM_CASCADES - 1].color * falloff * dotNL * PI;
        
                    #ifdef USE_CLEARCOAT        
                        ccDotNL = saturate(dot(clearcoatNormal, L));
                        ccIrradiance = ccDotNL * u_Directional[CSM_CASCADES - 1].color * falloff  * PI;
